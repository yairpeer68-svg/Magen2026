...
public final class VisualPolicy {
    ...
    public static Config get(Context c) {
        ...
        return new Config(
            ...
            // ברירות-מחדל מקומיות שהועלו כדי לצמצם false-positives בגלריה/תמונות
            // (עור/חוף/תינוקות חצו את הסף הישן 0.30 וזרקו מהגלריה). פורנוגרפיה
            // אמיתית מקבלת ציון גבוה בהרבה, ולכן העלאה זו כמעט אינה פוגעת בזיהוי.
            // אם השרת דוחף policy חתום — הערכים שלו גוברים על אלה.
            clampF(p.getFloat("porn_threshold", 0.45f), 0.05f, 0.99f),
            clampF(p.getFloat("hentai_threshold", 0.45f), 0.05f, 0.99f),
            clampF(p.getFloat("sexy_threshold", 0.55f), 0.05f, 0.99f),
            clampF(p.getFloat("unsafe_sum_threshold", 0.70f), 0.10f, 0.99f),
            ...
            clampF(p.getFloat("immediate_porn_threshold", 0.85f), 0.20f, 0.99f),
            clampF(p.getFloat("immediate_hentai_threshold", 0.85f), 0.20f, 0.99f),
            clampF(p.getFloat("immediate_sexy_threshold", 0.90f), 0.20f, 0.99f),
