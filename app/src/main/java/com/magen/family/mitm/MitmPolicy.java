...
public final class MitmPolicy {
    ...
    private static final Set<String> SENSITIVE_LABELS=new HashSet<>(Arrays.asList(
        "login","signin","signon","auth","oauth","sso","identity","account","accounts",
        "payment","payments","checkout","billing","wallet","bank","banking","onlinebanking",
        "health","medical","patient","patients","hospital","clinic","vault","password","passwords"
    ));
    private static final String[] SENSITIVE_FRAGMENTS={
        "bank","wallet","payment","checkout","medical","hospital","password","vault"
    };
    ...
    public static boolean shouldBypass(String host){
        ...
        // Generic privacy guard for providers not yet in the explicit lists. This examines DNS
        // labels only (never paths/content) and deliberately prefers tunneling over decryption.
        for(String label:h.split("\\.")){
            String compact=label.replace("-","").replace("_","");
            if(SENSITIVE_LABELS.contains(label)||SENSITIVE_LABELS.contains(compact))return true;
            for(String fragment:SENSITIVE_FRAGMENTS)if(compact.contains(fragment))return true;
        }
        return false;
    }
