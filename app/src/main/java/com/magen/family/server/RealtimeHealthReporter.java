...
public final class RealtimeHealthReporter {
    ...
    private static void runOnce() {
        ...
        // Piggyback the Circle-of-Strength notifications poll on the same cadence. It is a no-op
        // unless the user opted into the circle, and keeps an active helper "connected".
        try {
            if (app != null) com.magen.family.server.CircleClient.pollNotificationsBlocking(app);
        } catch (Throwable t) {
            Log.w(TAG, "circle poll failed: " + t.getClass().getSimpleName());
        }
