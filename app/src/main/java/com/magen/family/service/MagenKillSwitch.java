...
public class MagenKillSwitch extends Service {
    ...
    public static boolean start(Context ctx, Intent intent) {
        // בזמן הסרה-עצמית מכוונת אין להעלות מסך נעילה — הוא יחסום את דיאלוג
        // ההסרה. זו נקודת-החנק המרכזית: כל הקוראים (Tamper/onDisabled/block)
        // הופכים ל-no-op בזמן ההסרה.
        if (com.magen.family.admin.MagenSelfUninstall.inProgress(ctx)) return false;
        try {
