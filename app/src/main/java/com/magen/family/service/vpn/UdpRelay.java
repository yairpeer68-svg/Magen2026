...
public class UdpRelay {
    ...
    /** אם resolver לא ענה תוך הזמן הזה — עוברים ל-resolver המסנן הבא בשרשרת. */
    private static final long DNS_RETRY_MS       = 1_200L;
    ...
    private static class Session {
        ...
        // --- DNS failover ---
        byte[] dnsQuery;               // עותק השאילתה, לשליחה חוזרת ל-resolver הבא
        java.util.List<String> dnsChain;
        int dnsAttempt;                // אינדקס ה-resolver הנוכחי בשרשרת
        long dnsSentAt;                // מתי נשלחה השאילתה ל-resolver הנוכחי
        volatile boolean dnsAnswered;  // התקבלה תשובה כלשהי
    }
    ...
    private void forward(byte[] packet, int payloadOff, int payloadLen,
        ...
        // שמירת השאילתה כדי שנוכל לשלוח אותה שוב ל-resolver הבא אם הנוכחי לא עונה.
        if (isDns) {
            byte[] q = new byte[payloadLen];
            System.arraycopy(packet, payloadOff, q, 0, payloadLen);
            session.dnsQuery = q;
            session.dnsSentAt = session.lastUse;
            session.dnsAnswered = false;
        }
    ...
    private Session openSession(String key, int deviceIp, int devicePort,
        ...
        try {
            ...
            // Connect after VpnService.protect(). DNS sessions connect to the first filtered
            // resolver in the failover chain; ordinary UDP sessions connect to the original
            // destination. A connected DatagramChannel accepts inbound packets only from this peer.
            java.util.List<String> dnsChain = isDns ? VpnPolicy.upstreamDnsChain() : null;
            InetSocketAddress target = isDns
                ? new InetSocketAddress(dnsChain.get(0), 53)
    ...
    private void selectorLoop() {
        ...
        while (running) {
            try {
                registerPending();
                // בדיקת timeout קצרה כדי שנוכל לעבור ל-resolver הבא בזמן.
                selector.select(400);
    ...
    /**
     * DNS failover: כל שאילתת DNS שלא קיבלה תשובה תוך DNS_RETRY_MS נשלחת שוב
     * ל-resolver המסנן הבא בשרשרת. רץ רק על ה-thread של ה-selector.
     * בלי זה, resolver יחיד שאינו נגיש הפיל את כל פתרון-השמות → "אין אינטרנט".
     */
    private void retryStaleDns() {
        long now = SystemClock.elapsedRealtime();
        for (Session s : sessions.values()) {
            if (!s.isDns || s.dnsAnswered || s.dnsQuery == null || s.dnsChain == null) continue;
            if (now - s.dnsSentAt < DNS_RETRY_MS) continue;
            if (s.dnsAttempt >= s.dnsChain.size() - 1) continue;   // מיצינו את השרשרת

            s.dnsAttempt++;
            String next = s.dnsChain.get(s.dnsAttempt);
            try {
                s.channel.disconnect();
                s.channel.connect(new InetSocketAddress(next, 53));
                s.channel.write(ByteBuffer.wrap(s.dnsQuery));
                s.dnsSentAt = now;
                s.lastUse = now;
                Log.d(TAG, "DNS failover → " + next);
            } catch (Exception e) {
                Log.w(TAG, "DNS failover to " + next + " failed: " + e.getMessage());
                closeSession(s);
    ...
    private void readResponse(Session s, ByteBuffer buffer) {
        try {
            ...
            s.lastUse = SystemClock.elapsedRealtime();
            if (s.isDns) s.dnsAnswered = true;   // עצירת ה-failover ברגע שהגיעה תשובה
