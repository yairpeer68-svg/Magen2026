...
public class MagenAccessibilityService extends AccessibilityService {
    ...
    /**
     * אפליקציות יסוד שמצב "מחמיר" לא נועל. אלה פונקציות בסיס של הטלפון —
     * חייגן, אנשי קשר, מצלמה, גלריה/תמונות, שעון, הודעות SMS, קבצים והגדרות
     * מערכת. חסימתן הפכה את הטלפון לבלתי-שמיש במשך 15 דקות. סינון התוכן
     * (כולל סריקת תמונות בגלריה) עדיין פועל עליהן — זה רק מבטל את הנעילה
     * הגורפת של מצב מחמיר.
     */
    private static final Set<String> ESSENTIAL_APPS = new HashSet<>(Arrays.asList(
        ...
        "com.sec.android.app.camera", "com.oppo.camera", "com.coloros.camera", "com.oneplus.camera",
        "com.google.android.apps.photos",           // גלריה/תמונות — הבאג שזרק מהגלריה
        "com.android.gallery3d", "com.sec.android.gallery3d", "com.miui.gallery",
        "com.coloros.gallery3d", "com.oneplus.gallery",
        "com.google.android.apps.messaging", "com.android.messaging", "com.samsung.android.messaging",
        "com.android.mms",
        "com.android.documentsui", "com.google.android.documentsui",  // בורר קבצים
        "com.android.emergency", "com.google.android.apps.wellbeing"
    ));
    ...
    private void onAccessibilityEventSafe(AccessibilityEvent event) {
        ...
        if (event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            // מצב "מחמיר" מגביר את הסינון — אבל אסור לו לנעול את המשתמש מחוץ
            // לכל הטלפון. קודם הוא חסם *כל* אפליקציה שאינה phone/systemui/whitelist,
            // ולכן במשך 15 דקות אי אפשר היה להיכנס לגלריה, למצלמה, לאנשי קשר, לשעון
            // וכו' — כל פתיחת אפליקציה "זרקה החוצה". זה הבאג ש"חוסם את כל הטלפון".
            // עכשיו מצב מחמיר עדיין חוסם דפדפנים/רשתות-חברתיות/אפליקציות לא מזוהות,
            // אבל לא את פונקציות היסוד של הטלפון.
    ...
    private boolean handleSelfDefense(String pkg, String className) {
        // הסרה-עצמית מכוונת (כפתור "הסר את האפליקציה"): לא לחסום את מסך
        // מידע-האפליקציה / דיאלוג ההסרה שאנחנו עצמנו פתחנו.
        if (com.magen.family.admin.MagenSelfUninstall.inProgress(this)) return false;
        ...
        if (isSettingsPackage(pkg)) {
            ...
            if (root != null) {
                try {
                    boolean ourApp = isOurAppScreen(root);
                    ...
                    // מסך "DNS פרטי" (Private DNS / DoT). שינוי כאן ל-resolver
                    // שאינו מסנן הוא נתיב עקיפה — אין סיבה לגיטימית לגעת בו כשההגנה
                    // חמושה. (חסימת פורט 853 כבר מפילה DoT, וזה סוגר גם את המסך עצמו.)
                    boolean privateDnsScreen =
                        findText(root, "DNS פרטי") ||
                        findText(root, "Private DNS") ||
                        containsIgnoreCase(className, "privatedns");
                    if (armed && privateDnsScreen) {
                        if (behaviorAnalyzer != null) behaviorAnalyzer.recordVpnBypassAttempt();
                        ServerEventReporter.report(this, "PRIVATE_DNS_SETTINGS_BLOCKED", "HIGH", "private DNS settings");
                        block();
                        return true;
                    }
        ...
        if (pkg.contains("packageinstaller")) {
            ...
            if (root != null) {
                try {
                    // ⚠️ תיקון הבאג מהסרטון: דיאלוג ההסרה של המערכת ("להסיר את
                    // ההתקנה של האפליקציה הזו?") מציג את *שם ההסוואה* הנוכחי
                    // (למשל "רימון"/"גלריה"/"מחשבון"), לא "שומר הברית". לכן החיפוש
                    // הישן אחרי "שומר הברית" בלבד פספס אותו לגמרי, וההסרה עברה.
                    // isOurAppScreen מזהה גם את שם ההסוואה הפעיל ואת שם החבילה.
                    if (isOurAppScreen(root)) {
                        if (behaviorAnalyzer != null) behaviorAnalyzer.recordUninstallAttempt();
    ...
    /**
     * זיהוי מסך שקשור לאפליקציה שלנו — עמיד להסוואה.
     *
     * הבעיה הקודמת: ההגנה חיפשה רק את המחרוזות הקבועות "שומר הברית"/"Magen"/
     * "magen.family". אבל האפליקציה מוסווית: במסך ההסרה, במידע-על-האפליקציה
     * ובמסך ההרשאות המערכת מציגה את *שם ההסוואה הפעיל* (למשל "רימון",
     * "גלריה", "מחשבון") ואת שם החבילה. לכן דיאלוג ההסרה מהסרטון לא זוהה
     * וההסרה עברה. כאן אנחנו מזהים לפי:
     *   • שם החבילה (com.magen.family) — מופיע במסך מידע-על-האפליקציה,
     *   • השמות הקבועים,
     *   • שם ההסוואה הפעיל הנוכחי (נקרא בזמן ריצה מ-DisguiseManager).
     */
    private boolean isOurAppScreen(AccessibilityNodeInfo root) {
        if (root == null) return false;
        if (findText(root, getPackageName())) return true;      // "com.magen.family"
        if (findText(root, "שומר הברית") || findText(root, "Magen")
                || findText(root, "magen.family")) return true;
        String disguise = currentDisguiseLabel();
        return disguise != null && !disguise.isEmpty() && findText(root, disguise);
    }

    /** שם ההסוואה הפעיל כרגע, או null. שמור מפני חריגות/היעדר משאב. */
    private String currentDisguiseLabel() {
        try {
            int idx = com.magen.family.disguise.DisguiseManager.currentIndex(this);
            if (idx < 0 || idx >= com.magen.family.disguise.DisguiseManager.ALL.size()) return null;
            int labelRes = com.magen.family.disguise.DisguiseManager.ALL.get(idx).labelRes;
    ...
    /**
     * אפליקציות יסוד של הטלפון שאסור למצב "מחמיר" לנעול מהמשתמש.
     * מצב מחמיר עדיין חוסם דפדפנים/רשתות-חברתיות/אפליקציות לא מזוהות, אבל לא
     * את חייגן, אנשי קשר, מצלמה, גלריה/תמונות, שעון, הודעות, המשגר וההגדרות.
     */
    private boolean isEssentialApp(String pkg) {
        if (pkg == null || pkg.isEmpty()) return false;
        if (ESSENTIAL_APPS.contains(pkg)) return true;
        String l = pkg.toLowerCase(java.util.Locale.ROOT);
        return l.endsWith(".launcher") || l.contains(".launcher.")
            || l.contains("inputmethod")            // מקלדת — נעילה שלה מקפיאה הקלדה
            || l.contains("dialer") || l.contains("contacts")
            || l.contains("deskclock") || l.contains("clock")
            || l.contains("camera") || l.contains("gallery")
    ...
    private boolean handleProtectionDisableConfirmation() {
        ...
        if (root == null) return false;
        try {
            boolean ourApp = isOurAppScreen(root) || findText(root, "סינון תוכן");
    ...
    /** מזהי view של שורת-הכתובת בדפדפנים נפוצים (סיומת אחרי ":id/"). */
    private static final String[] URL_BAR_VIEW_IDS = {
        "url_bar",                          // Chromium: Chrome/Edge/Brave/Kiwi/Vivaldi/Opera/Yandex ועוד
        "location_bar_edit_text",           // Samsung Internet
        "mozac_browser_toolbar_url_view",   // Firefox (GeckoView) + Focus
        "url",                              // כמה דפדפנים קלים
        "url_field", "addressbarEdit",      // UC / Via
        "search_edit_text", "address"       // דפדפני יצרן שונים
    };
    ...
    private String extractUrl() {
        ...
        try {
            ...
            // 1. דינמי לפי החבילה הנוכחית — מכסה אוטומטית כל fork של Chromium/Firefox
            //    (Opera, Brave, Kiwi, Vivaldi, Yandex, DuckDuckGo, Mi, Huawei וכו')
            //    בלי צורך ברשימה קבועה של כל דפדפן.
            if (pkg != null && !pkg.isEmpty()) {
                for (String id : URL_BAR_VIEW_IDS) {
                    String url = getViewText(root, pkg + ":id/" + id);
                    if (url != null && !url.isEmpty()) return url;
                }
            }

            // 2. גיבוי — מזהים מלאים מפורשים (למקרה שהחבילה שונה מ-namespace ה-id)
