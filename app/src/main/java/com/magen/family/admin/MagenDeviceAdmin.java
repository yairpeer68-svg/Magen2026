...
public class MagenDeviceAdmin extends DeviceAdminReceiver {
    ...
    public void onDisabled(Context context, Intent intent) {
        ...
        // הסרה-עצמית מכוונת: אנחנו בעצמנו הסרנו את מנהל-המכשיר כחלק מכפתור
        // "הסר את האפליקציה". אין להריע ולא להקים KillSwitch — פשוט לצאת.
        if (MagenSelfUninstall.inProgress(context)) {
            Log.d(TAG, "Device Admin disabled as part of self-uninstall — no re-lock");
            return;
        }
    ...
    public static boolean lockDeviceNow(Context context) {
        // בזמן הסרה-עצמית מכוונת אסור לנעול את המסך — אחרת "כפתור ההסרה"
        // ננעל את המכשיר בדיוק כשמסירים את מנהל-המכשיר.
        if (MagenSelfUninstall.inProgress(context)) return false;
        try {
