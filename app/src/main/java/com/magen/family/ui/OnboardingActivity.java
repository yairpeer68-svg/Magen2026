...
public class OnboardingActivity extends BaseActivity {
    ...
    public static final String EXTRA_BOOTSTRAP_ONLY = "permission_bootstrap_only";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bootstrapOnly = getIntent().getBooleanExtra(EXTRA_BOOTSTRAP_ONLY, false);
        buildSteps();
    ...
    private void buildSteps() {
        // First screen is the complete capability matrix. On first app launch this is
        // intentionally shown before name/PIN setup so permissions are never hidden behind menus.
        steps.add(new Step(R.string.onb_permissions_all_title, R.string.onb_permissions_all_body,
            null, null, false));

        if (!bootstrapOnly) {
            steps.add(new Step(R.string.onb_how_title, R.string.onb_how_body, null, null, false));
        }
        ...
        // Self-update permission. Android treats this as Special App Access, not a runtime permission.
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            steps.add(new Step(R.string.onb_perm_updates_title, R.string.onb_perm_updates_body,
                () -> getPackageManager().canRequestPackageInstalls(),
                () -> com.magen.family.util.SafeLaunch.open(this,
                    com.magen.family.util.PermissionMatrix.unknownSourcesIntent(this)), false));
        }

        // Device Owner is stronger than Device Admin but cannot be granted by a normal app.
        // We display capability status instead of pretending Android can grant it here.
        steps.add(new Step(R.string.onb_perm_device_owner_title, R.string.onb_perm_device_owner_body,
            () -> com.magen.family.util.PermissionMatrix.deviceOwner(this),
            this::showDeviceOwnerInstructions, false));
    ...
    private void render() {
        ...
        if (informational) {
            if (s.title == R.string.onb_permissions_all_title) {
                int grantedCount=com.magen.family.util.PermissionMatrix.grantedCount(this);
                int totalCount=com.magen.family.util.PermissionMatrix.totalCount(this);
                tvStatus.setText(getString(R.string.onb_permissions_summary, grantedCount, totalCount));
    ...
    private String guardScopeForStep(Step s) {
        ...
        if (s.title == R.string.onb_perm_updates_title)
            return com.magen.family.service.MagenGuard.SCOPE_APP_DETAILS;
        if (s.title == R.string.onb_perm_device_owner_title)
    ...
    private void finishOnboarding() {
        ...
        MagenApp.getInstance().getPrefs().edit()
            ...
            .putBoolean("onboarding_done", !bootstrapOnly).apply();
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        // Runtime permission dialogs can finish without another onResume. Continue
        // automatically when the permission was actually granted.
        if (!advanceIfGrantedAfterAttempt()) render();
    }
    ...
    private boolean advanceIfGrantedAfterAttempt() {
        try {
            if (index < 0 || index >= steps.size() || !attempted.contains(index)) return false;
            Step s=steps.get(index);
            if (s.check==null || !s.check.granted()) return false;
    ...
    private void showDeviceOwnerInstructions() {
        String cmd="adb shell dpm set-device-owner " + getPackageName() + "/.admin.MagenDeviceAdmin";
        new androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle(R.string.onb_perm_device_owner_title)
            .setMessage(getString(R.string.onb_device_owner_instructions, cmd))
