...
findViewById(R.id.btn_screen_time).setOnClickListener(v -> askPin(REQ_PIN_SCREEN_TIME));
    ...
    protected void onActivityResult(int req, int res, Intent data) {
        ...
        if (req == REQ_PIN_TOGGLE) {
            ...
            if (next) {
                // הפעלה — מיידית, וגם מבטלת בקשת כיבוי ממתינה אם הייתה.
                com.magen.family.service.MagenCooldown.cancel(this);
                MagenApp.getInstance().getPrefs()
                    .edit().putBoolean(MagenApp.KEY_FILTER_ENABLED, true).apply();
                refreshStatus();
            } else {
                // כיבוי — עובר דרך מנגנון ההשהיה (cooldown) לשמירה עצמית.
    ...
    private void showAdvancedDialog() {
        ...
        // ---- השהיית כיבוי/הסרה (cooldown) ----
        Button btnCooldown = new Button(this);
        btnCooldown.setAllCaps(false);
        btnCooldown.setText("⏳ השהיית כיבוי (cooldown)");
        btnCooldown.setPadding(0, pad / 2, 0, 0);
        root.addView(btnCooldown);
        btnCooldown.setOnClickListener(v -> showCooldownDialog());

        // ---- הסרה-עצמית נקייה (המסך הזה כבר מאחורי PIN) ----
        Button btnUninstall = new Button(this);
        btnUninstall.setAllCaps(false);
        btnUninstall.setText("🗑️ הסר את האפליקציה");
        btnUninstall.setTextColor(0xFFD32F2F);
        btnUninstall.setPadding(0, pad / 2, 0, 0);
        root.addView(btnUninstall);
        btnUninstall.setOnClickListener(v -> confirmSelfUninstall());
    ...
    /**
     * בקשת כיבוי ההגנה — עוברת דרך מנגנון ההשהיה (cooldown) לשמירה עצמית.
     * אם ההשהיה כבויה (0 דק') — כיבוי מיידי כמו קודם.
     */
    private void handleDisableRequest() {
        if (!com.magen.family.service.MagenCooldown.enabled(this)) {
            MagenApp.getInstance().getPrefs()
                .edit().putBoolean(MagenApp.KEY_FILTER_ENABLED, false).apply();
            refreshStatus();
            return;
        }
        if (com.magen.family.service.MagenCooldown.isElapsed(this, com.magen.family.service.MagenCooldown.TYPE_DISABLE)) {
            // ההשהיה הסתיימה — מכבים בפועל ומנקים את הבקשה.
            com.magen.family.service.MagenCooldown.cancel(this);
            MagenApp.getInstance().getPrefs()
                .edit().putBoolean(MagenApp.KEY_FILTER_ENABLED, false).apply();
            refreshStatus();
            Toast.makeText(this, "ההגנה כובתה.", Toast.LENGTH_LONG).show();
            return;
        }
        if (com.magen.family.service.MagenCooldown.isPending(this, com.magen.family.service.MagenCooldown.TYPE_DISABLE)) {
            showCooldownPendingDialog("כיבוי ההגנה");
            return;
        }
        // בקשה חדשה
        int hours = Math.max(1, com.magen.family.service.MagenCooldown.configuredMinutes(this) / 60);
        com.magen.family.service.MagenCooldown.request(this, com.magen.family.service.MagenCooldown.TYPE_DISABLE);
        new android.app.AlertDialog.Builder(this)
            .setTitle("בקשת כיבוי נרשמה")
            .setMessage("ההגנה תישאר פעילה ותיכבה רק בעוד כ-" + hours + " שעות.\n"
                + "אפשר לבטל את הבקשה בכל רגע. זה נועד לעזור לך לעבור את רגע החולשה.")
    ...
    /**
     * הגדרת משך ההשהיה + צפייה/ביטול בקשה ממתינה.
     * העלאת המשך (או ביטול בקשה) הן פעולות "מקילות" ומותרות מיד; המנגנון
     * מגן דווקא מפני *כיבוי* מהיר, לא מפני החמרה.
     */
    private void showCooldownDialog() {
        final int[] hoursOptions = {0, 1, 3, 6, 12, 24, 48, 72};
        final String[] labels = new String[hoursOptions.length];
        int current = com.magen.family.service.MagenCooldown.configuredMinutes(this) / 60;
        int checked = 4; // ברירת מחדל 12ש'
        for (int i = 0; i < hoursOptions.length; i++) {
            labels[i] = hoursOptions[i] == 0 ? "כבוי (מיידי)" : hoursOptions[i] + " שעות";
            if (hoursOptions[i] == current) checked = i;
        }

        String pendingInfo;
        String pt = com.magen.family.service.MagenCooldown.requestType(this);
        if (pt == null) {
            pendingInfo = "אין בקשת כיבוי/הסרה ממתינה.";
        } else {
            String what = com.magen.family.service.MagenCooldown.TYPE_UNINSTALL.equals(pt)
                ? "הסרת האפליקציה" : "כיבוי ההגנה";
            long rem = com.magen.family.service.MagenCooldown.remainingMs(this);
            pendingInfo = "בקשה ממתינה: " + what + (rem > 0
                ? " — עוד " + com.magen.family.service.MagenCooldown.formatRemaining(this)
                : " — מוכנה לביצוע");
        }

        final int[] sel = { checked };
        android.app.AlertDialog.Builder b = new android.app.AlertDialog.Builder(this)
            .setTitle("השהיית כיבוי")
            .setSingleChoiceItems(labels, checked, (d, w) -> sel[0] = w)
            .setMessage(pendingInfo)
            .setPositiveButton("שמור", (d, w) -> {
                com.magen.family.service.MagenCooldown.setConfiguredMinutes(this, hoursOptions[sel[0]] * 60);
                Toast.makeText(this, "✓ נשמר", Toast.LENGTH_SHORT).show();
                refreshStatus();
            })
            .setNegativeButton(R.string.cancel, null);
        if (pt != null) {
            b.setNeutralButton("בטל בקשה ממתינה", (d, w) -> {
                com.magen.family.service.MagenCooldown.cancel(this);
                Toast.makeText(this, "✓ הבקשה בוטלה", Toast.LENGTH_SHORT).show();
                refreshStatus();
            });
        }
        b.show();
    ...
    private void showCooldownPendingDialog(String actionLabel) {
        String remaining = com.magen.family.service.MagenCooldown.formatRemaining(this);
        new android.app.AlertDialog.Builder(this)
            .setTitle("בהמתנה: " + actionLabel)
            .setMessage("הפעולה תתאפשר בעוד " + remaining + ".\nההגנה פעילה עד אז.")
            .setNegativeButton("סגור", null)
            .setPositiveButton("בטל את הבקשה", (d, w) -> {
                com.magen.family.service.MagenCooldown.cancel(this);
                Toast.makeText(this, "✓ הבקשה בוטלה. ההגנה נשארת.", Toast.LENGTH_LONG).show();
                refreshStatus();
            })
            .show();
    }
    ...
    /**
     * אישור לפני הסרה-עצמית. עובר דרך מנגנון ההשהיה: בקשת הסרה ראשונה רק
     * *מתחילה* את הטיימר; ההסרה בפועל מתאפשרת רק אחרי שהזמן עבר. אם ההשהיה
     * כבויה (0 דק') — הסרה מיידית אחרי אישור.
     */
    private void confirmSelfUninstall() {
        if (com.magen.family.service.MagenCooldown.enabled(this)
                && !com.magen.family.service.MagenCooldown.isElapsed(this,
                        com.magen.family.service.MagenCooldown.TYPE_UNINSTALL)) {
            if (com.magen.family.service.MagenCooldown.isPending(this,
                    com.magen.family.service.MagenCooldown.TYPE_UNINSTALL)) {
                showCooldownPendingDialog("הסרת האפליקציה");
                return;
            }
            int hours = Math.max(1, com.magen.family.service.MagenCooldown.configuredMinutes(this) / 60);
            com.magen.family.service.MagenCooldown.request(this,
                com.magen.family.service.MagenCooldown.TYPE_UNINSTALL);
            new android.app.AlertDialog.Builder(this)
                .setTitle("בקשת הסרה נרשמה")
                .setMessage("האפליקציה תישאר מותקנת ותהיה ניתנת להסרה רק בעוד כ-" + hours
                    + " שעות. אפשר לבטל את הבקשה בכל רגע.")
                .setPositiveButton("הבנתי", null)
                .show();
            return;
        }
        // ההשהיה עברה (או כבויה) — אישור אחרון והסרה בפועל.
        new android.app.AlertDialog.Builder(this)
            .setTitle("הסרת האפליקציה")
            .setMessage("הפעולה תכבה את ההגנה, תסיר את מנהל-המכשיר ותפתח את מסך "
                + "ההסרה של המערכת. להמשיך?")
            .setNegativeButton(R.string.cancel, null)
            .setPositiveButton("הסר", (d, w) -> {
                com.magen.family.service.MagenCooldown.cancel(this);
                com.magen.family.admin.MagenSelfUninstall.start(this);
            })
            .show();
    }
