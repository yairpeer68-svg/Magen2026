...
def check_java_import_collisions():
    """Detect imports that make real javac compilation fail (same simple class name twice)."""
    global checks_run
    checks_run += 1
    scanned = 0
    for path in java_files(SRC, TEST):
        scanned += 1
        imports = re.findall(r'^\s*import\s+(?:static\s+)?([A-Za-z0-9_.$]+)\s*;', read(path), re.M)
        seen = {}
        for fq in imports:
            simple = fq.rsplit('.', 1)[-1]
            if simple == '*':
                continue
            prev = seen.get(simple)
            if prev and prev != fq:
                error("Java imports", f"{rel(path)}: import collision for {simple}: {prev} vs {fq}")
            seen[simple] = fq
    return f"{scanned} קבצי Java"
...
def check_v452_shortform_autoskip():
    ...
    if not os.path.isfile(guard_path):
        error('v4.6.0', 'ShortFormSkipGuard חסר')
    ...
    for token in ('WAITING_FOR_ADVANCE', 'MAX_SKIPS_PER_BURST', 'CIRCUIT_BREAK_MS', 'markScrolled', 'markGestureFailed'):
        if token not in guard: error('v4.6.0', 'anti-loop guard חסר: '+token)
    for token in ('dispatchGesture(', 'showAutoSkip(', 'SHORTFORM_AUTOSKIP_CIRCUIT', 'shortFormSkipGuard.markScrolled'):
        if token not in svc: error('v4.6.0', 'one-shot short-form skip חסר: '+token)
    if 'android:canPerformGestures="true"' not in xml:
        error('v4.6.0', 'Accessibility Service אינו מורשה לבצע gesture')
    if 'FLAG_NOT_TOUCHABLE' not in curtain or 'showAutoSkip' not in curtain:
        error('v4.6.0', 'auto-skip curtain חייב להיות pass-through/non-touchable')
    if 'versionName "4.7.1-hardening-fix-8443"' not in gradle:
        error('v4.7.1', 'versionName לא עודכן')
    if not os.path.isfile(test_path):
        error('v4.6.0', 'חסרות בדיקות ShortFormSkipGuard')
...
def check_v470_permissions_release_trust():
    manifest=read(os.path.join(ROOT,'app','src','main','AndroidManifest.xml'))
    welcome=read(os.path.join(SRC,'com','magen','family','ui','WelcomeActivity.java'))
    onboarding=read(os.path.join(SRC,'com','magen','family','ui','OnboardingActivity.java'))
    matrix_path=os.path.join(SRC,'com','magen','family','util','PermissionMatrix.java')
    update=read(os.path.join(SRC,'com','magen','family','service','UpdateChecker.java'))
    hb=read(os.path.join(SRC,'com','magen','family','server','HeartbeatManager.java'))
    identity=read(os.path.join(SRC,'com','magen','family','server','DeviceIdentity.java'))
    verifier=read(os.path.join(SRC,'com','magen','family','server','ServerResponseVerifier.java'))
    gradle=read(os.path.join(ROOT,'app','build.gradle'))
    if not os.path.isfile(matrix_path): error('v4.7.1','PermissionMatrix חסר')
    else:
        matrix=read(matrix_path)
        for token in ('SCHEMA_VERSION','canInstallUpdates','deviceOwner','coreReady','backgroundLocation'):
            if token not in matrix: error('v4.7.1','PermissionMatrix חסר: '+token)
    for token in ('EXTRA_BOOTSTRAP_ONLY','permission_bootstrap_version','PERMISSIONS_REQUEST_CODE'):
        if token not in welcome: error('v4.7.1','first-launch permission gate חסר: '+token)
    for token in ('onb_perm_updates_title','onb_perm_device_owner_title','ACTION_ACCESSIBILITY_SETTINGS','ACTION_USAGE_ACCESS_SETTINGS','VpnService.prepare'):
        if token not in onboarding: error('v4.7.1','onboarding capability חסר: '+token)
    # Do not inflate the app's attack/privacy surface by requesting unrelated dangerous permissions.
    for forbidden in ('android.permission.CAMERA','android.permission.RECORD_AUDIO','android.permission.READ_CONTACTS',
                      'android.permission.READ_SMS','android.permission.CALL_PHONE','android.permission.MANAGE_EXTERNAL_STORAGE'):
        if forbidden in manifest: error('v4.7.1','הרשאה פולשנית שאינה נדרשת נוספה: '+forbidden)
    for token in ('K_MAX_SEQUENCE_PREFIX','K_MAX_SEQUENCE_DIGEST_PREFIX','rolloutPercent','release_channel','UPDATE_NEXT_PUBKEY_B64'):
        if token not in update: error('v4.7.1','OTA hardening חסר: '+token)
    if not re.search(r'private\s+static\s+int\s+rolloutBucket\s*\(', update):
        error('v4.7.1','rolloutBucket קיים כקריאה אך אין לו implementation')
    if not re.search(r'private\s+static\s+boolean\s+verifyManifestWithKey\s*\(', update):
        error('v4.7.1','verifyManifestWithKey קיים כקריאה אך אין לו implementation')
    for token in ('SHA256withECDSA','ECPublicKey', 'secp256r1','persistManifestWatermark','sequenceDigestKey','commit()'):
        if token not in update: error('v4.7.1','OTA implementation חסר: '+token)
    if 'MAGEN_SERVER_SIGNING_NEXT_PUB_B64' not in verifier:
        error('v4.7.1','server signing key rotation trust slot חסר')
    for token in ('isHardwareBacked','isStrongBoxBacked'):
        if token not in identity: error('v4.7.1','keystore trust signal חסר: '+token)
    for token in ('hardware_key_backed','root_suspected','permission_core_ready','trust_score'):
        if token not in hb: error('v4.7.1','heartbeat trust signal חסר: '+token)
    if 'versionCode 23' not in gradle: error('v4.7.1','versionCode 23 חסר')
    return 'first-launch permission matrix / least privilege / OTA anti-replay+channels / key rotation / trust telemetry'
...
def main():
    ...
    checks = [
        ...
        ("Java import collisions", check_java_import_collisions),
        ...
        ("Short-form auto-skip + release v4.7.1", check_v452_shortform_autoskip),
        ("v4.7 permissions/release", check_v470_permissions_release_trust),
