...
Info 'Windows APK builder v4.7.1-hardening-fix-8443'
...
if (-not (Test-Path $VisualModel)) {
    ...
    try {
        $release = Invoke-RestMethod -Uri 'https://api.github.com/repos/GantMan/nsfw_model/releases/tags/1.1.0' -Headers @{ 'User-Agent'='MagenBuilder/4.7.1' }
...
if (Test-Path $ModelLock) {
    ...
    $ExpectedModelHash = if ($env:MAGEN_VISUAL_MODEL_SHA256) { $env:MAGEN_VISUAL_MODEL_SHA256.Trim().ToLowerInvariant().Replace(':','') } else { '' }
    if ($ExpectedModelHash) {
        if ($ExpectedModelHash -notmatch '^[0-9a-f]{64}$') { Fail 'MAGEN_VISUAL_MODEL_SHA256 must be exactly 64 hex characters.' }
        if ($ModelHash -ne $ExpectedModelHash) { Fail "Visual model SHA256 mismatch: expected $ExpectedModelHash got $ModelHash" }
    } elseif (-not $archiveVerified) {
        Fail 'No repository model lock and GitHub supplied no archive digest. Set MAGEN_VISUAL_MODEL_SHA256 from an independently audited model before the first production build.'
    }
    $ModelHash | Set-Content -Path $ModelLock -Encoding ascii
    Write-Host '[Visual AI] verified model lock created. Commit VISUAL_MODEL_SHA256.lock to the repository.' -ForegroundColor Green
}
...
if (Test-Path $pairingPropsPath) {
    ...
    if ($pair.ContainsKey('magenServerSigningNextPublicKey')) { $env:MAGEN_SERVER_SIGNING_NEXT_PUBLIC_KEY = $pair['magenServerSigningNextPublicKey'] }
...
$outName = 'magen-v4.7.1-hardening-fix-8443-release.apk'
